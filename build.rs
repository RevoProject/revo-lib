...
fn main() -> Result<(), Box<dyn Error>> {
   ...
   // 3) Configure OBS
    ...
    if cfg!(target_os = "windows") {
        ...
        fs::write(&shim, r#"/* Auto-generated by build.rs – stub out all MSVC SIMD intrinsic headers.
 * We define minimal opaque structs for the vector types that obs headers
 * may transitively reference, then pre-define every intrinsic include-guard
 * so clang's preprocessor never enters those header bodies.
 */
typedef struct __declspec(align(8))  { unsigned char b[8];  } __m64;
typedef struct __declspec(align(16)) { unsigned char b[16]; } __m128;
typedef struct __declspec(align(16)) { unsigned char b[16]; } __m128d;
typedef struct __declspec(align(16)) { unsigned char b[16]; } __m128i;
typedef struct __declspec(align(32)) { unsigned char b[32]; } __m256;
typedef struct __declspec(align(32)) { unsigned char b[32]; } __m256d;
typedef struct __declspec(align(32)) { unsigned char b[32]; } __m256i;
typedef struct __declspec(align(64)) { unsigned char b[64]; } __m512;
...
"#)?;
        bindgen_builder = bindgen_builder
            ...
            // Inject the shim before every other header so all intrinsic guards
            // are pre-defined and the vector types are visible:
            .clang_arg("-include")
            .clang_arg(shim.to_string_lossy().into_owned())
            // Belt-and-suspenders: also blocklist so nothing slips through:
