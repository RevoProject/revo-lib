...
fn main() -> Result<(), Box<dyn Error>> {
   ...
   // 3) Configure OBS
    ...
    // On Windows, MSVC's bundled Clang 19 cannot parse its own SIMD intrinsic
    // headers (mmintrin.h has __m64<->int issues; ammintrin.h/fma4intrin.h
    // depend on vector types from headers we'd have to chain-block, etc.).
    // Solution: write a shim header that (a) defines minimal struct stubs for
    // all SIMD vector types, then (b) pre-defines every intrinsic header's
    // include-guard so clang's preprocessor skips all of them entirely.
    // Inject the shim via `-include` before everything else.
    if cfg!(target_os = "windows") {
        let out_dir_early = PathBuf::from(env::var("OUT_DIR")?);
        let shim = out_dir_early.join("bindgen_intrin_shim.h");
        // Only mmintrin.h is broken (Clang 19 __m64 vector<->int mismatch).
        // All other intrinsic headers (xmmintrin, emmintrin, etc.) must parse
        // normally because wchar.h and OBS vec4.h call _mm_* intrinsics and
        // assign __m128/__m128i values.  Strategy:
        //   1. Define __m64 as a plain long long (fully compatible with how
        //      xmmintrin.h uses it as a parameter/return type).
        //   2. Pre-define __MMINTRIN_H so mmintrin.h's body is never parsed
        //      (it would redefine __m64 as a vector type and error out).
        //   3. Leave every other intrinsic header unblocked.
        fs::write(&shim, r#"/* Auto-generated by build.rs.
 * Clang 19 mmintrin.h defines __m64 as a GNU vector type and then does
 * int<->vector assignments that Clang itself rejects.  We block mmintrin.h's
 * body via its include guard and provide the pieces that xmmintrin.h needs:
 *   - __m64            as a plain scalar long long
 *   - __v2si/__v4hi/__v8qi  as proper __vector_size__ types (xmmintrin.h uses
 *                           these at lines ~1680-2568 for MMX-blend intrinsics)
 * All other SIMD headers (xmmintrin, emmintrin, immintrin ...) are left
 * untouched so that wchar.h / vec4.h / etc. can call real _mm_* builtins.
 */
#ifndef __M64__
#define __M64__
typedef long long __m64;
#endif
/* Internal MMX vector element types normally defined in mmintrin.h's body.
 * xmmintrin.h (which we do NOT block) needs them. */
typedef int   __v2si  __attribute__((__vector_size__(8)));
typedef short __v4hi  __attribute__((__vector_size__(8)));
typedef char  __v8qi  __attribute__((__vector_size__(8)));
#define __MMINTRIN_H  /* skip the broken mmintrin.h body */
"#)?;
        bindgen_builder = bindgen_builder
            .clang_arg("--target=x86_64-pc-windows-msvc")
            // Inject the shim before every other header:
            .clang_arg("-include")
            .clang_arg(shim.to_string_lossy().into_owned())
            // Blocklist mmintrin items from bindgen output (belt-and-suspenders):
